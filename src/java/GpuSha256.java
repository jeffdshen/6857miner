...
//            throw new IllegalArgumentException("Input data length must be 40 bytes");
    ...
    public boolean hash(long nonce, int difficulty) {
        ...
        for (r = 16; r < 64; r += 16) {
            ...
            D += T;
            H = T + T2;
            w1 += (((w15 >>> 17) | (w15 << (32 - 17))) ^ ((w15 >>> 19)
                | (w15 << (32 - 19))) ^ (w15 >>> 10)) + w10
                + (((w2 >>> 7) | (w2 << (32 - 7)))
                ^ ((w2 >>> 18) | (w2 << (32 - 18)))
                ^ (w2 >>> 3));
            T = G + (((D >>> 6) | (D << (32 - 6))) ^ ((D >>> 11)
