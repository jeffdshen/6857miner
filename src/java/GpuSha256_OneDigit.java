...
*/
public class GpuSha256_OneDigit extends Kernel {
...
//    public final long[] count = new long[8192];
     ...
     *
     */
    public GpuSha256_OneDigit(byte[] block, long start, long mod, int length) {
        this.start = start;
        this.mod = mod;
        this.length = length;
        setInput(block);
...
//    }
     ...
     *
     * @param       inputBytes      Bytes to be hashed (45 bytes)
     */
    public void setInput(byte[] inputBytes) {
//        if (inputBytes.length != 45)
//            throw new IllegalArgumentException("Input data length must be 45 bytes");
        //
        // Copy the input data
        //


        // CUSTOM : assumes content = 1 bytes, and thus we have 45 total bytes
        System.arraycopy(inputBytes, 0, input, 0, 45);
        //
        // Pad the buffer
        //
        ...
        // 45 bytes of data, the data bit count is 368 (0x168).
        //
        input[45] = (byte)0x80;
        input[62] = (byte)0x01;
        input[63] = (byte)0x68;
    ...
    public boolean hash(long nonce, int difficulty) {
        ...
        // unroll: r = 8
        // CUSTOM : nonce at bytes 33, 34, 35 (33 / 4 = 8)
        w16 = (input[32] << 24 | (nonce0 & 0xFF) << 16 |
            (nonce1 & 0xFF) << 8 | (nonce2 & 0xFF));
        T = (H + (((E >>> 6) | (E << 26)) ^ ((E >>> 11) | (E << 21)) ^ ((E >>> 25) | (E << 7))) +
            ((E & F) ^ (~E & G)) + k[8] + w16);
        T2 = ((((A >>> 2) | (A << 30)) ^ ((A >>> 13) | (A << 19)) ^ ((A >>> 22) | (A << 10))) +
            ((A & B) ^ (A & C) ^ (B & C)));
        ...
        w16 = (nonce3 << 24 | (nonce4 & 0xFF) << 16 |
            (nonce5 & 0xFF) << 8 | (nonce6 & 0xFF));
        T = (H + (((E >>> 6) | (E << 26)) ^ ((E >>> 11) | (E << 21)) ^ ((E >>> 25) | (E << 7))) +
            ((E & F) ^ (~E & G)) + k[9] + w16);
        T2 = ((((A >>> 2) | (A << 30)) ^ ((A >>> 13) | (A << 19)) ^ ((A >>> 22) | (A << 10))) +
            ((A & B) ^ (A & C) ^ (B & C)));
        ...
        // unroll: r = 10
        // CUSTOM: nonce at byte 40 (40 / 4 = 10)
        w16 = (nonce7 << 24 | (input[41] & 0xFF) << 16 |
